...
//#####################################
bool CGPRS_SIM800::init(unsigned int timeout)
{   
    SIM_SERIAL.begin(9600);
#if DEB_data
	Serial.println("21-03-20 SIM800_beelogger_22.h");
	Serial.flush();
#endif
...
//#####################################
byte CGPRS_SIM800::setup(const char* apn, const char* apn_benutzer, const char* apn_passwort)
{
  bool success = false;
  byte n =0;
  do {
...
//#####################################
bool CGPRS_SIM800::httpInit(unsigned int timeout)
{
  uint32_t t = millis();
  do {	
...
//#####################################
// Start Task to APN
byte CGPRS_SIM800::start(const char* apn)
{
  bool success = false;
  byte n = 0; 
  do {
  ...
  char c_cmd[16];
  strcpy_P(c_cmd, GSM_TASK);
  strcpy(buffer, c_cmd); 	   // Start Task to APN
  strcat(buffer, apn);
  strcat(buffer, "\"");
  if (sendCommand(buffer, 10000)) {
...
bool CGPRS_SIM800::Connect(const char* host_name)
{
  char c_cmd[24];
  strcpy_P(c_cmd, CIP_START);
  strcpy(buffer, c_cmd); 	// connect to hostname
  strcat(buffer, host_name);
  strcat(buffer, "\",80");     	// port 80
  // set up or check for a connection
  if (sendCommand(buffer, "CONNECT OK","ALREADY", 10000)) {
    return true;
  }
  disConnect();
  return false;
